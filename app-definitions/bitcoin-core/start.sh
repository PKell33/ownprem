...
if [ "${DEV_MODE:-}" = "true" ] || [ ! -w "/opt" ]; then
  ...
  mkdir -p "$LOG_DIR"
  LOG_FILE="${LOG_DIR}/bitcoin-core.log"

  echo "$(date -Iseconds) [INFO] Bitcoin Core started in dev/mock mode" >> "$LOG_FILE"
  ...
  while true; do
    echo "$(date -Iseconds) [INFO] Heartbeat - mock Bitcoin Core is alive" >> "$LOG_FILE"
    sleep 60
  done &
  exit 0
fi
